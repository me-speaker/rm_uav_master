...
```bash
# (1) 清理: 杀掉之前 fake 测试残留
docker exec rm_dep pkill -f fake_odom_publisher 2>/dev/null
docker exec rm_dep pkill -f slam_to_mavros_node 2>/dev/null
...
```bash
docker exec rm_dep lsusb | grep -i 2207
```
...
```bash
docker exec -d rm_dep bash -lc "source /opt/uav_ws/install/setup.bash && \
...
```bash
docker exec rm_dep bash -c "source /opt/uav_ws/install/setup.bash && \
...
**没数据?** 看 driver 日志:
```bash
docker exec rm_dep tail -80 /tmp/odin.log
```
...
```bash
# (1) 开启建图
docker exec rm_dep bash -c "echo 'set save_map 1' > /tmp/odin_command.txt"

# (2) 手持 ODIN (或装在飞机上) 绕场地走一圈, 速度 < 0.5 m/s
#     覆盖主要区域 + 转弯处. 大约 30-60s.

# (3) 停止建图, .bin 文件会被保存
docker exec rm_dep bash -c "echo 'set save_map 0' > /tmp/odin_command.txt"

# (4) 找地图文件
docker exec rm_dep find / -name "*.bin" 2>/dev/null | grep -i odin | head -5
```
...
```bash
docker exec rm_dep bash -lc "source /opt/uav_ws/install/setup.bash && \
...
| mavros 反复重启 | 检查 PX4 USB 还在; `docker logs rm_dep` 看错误 |
